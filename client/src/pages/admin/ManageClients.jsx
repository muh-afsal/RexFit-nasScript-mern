...
const ManageClients = () => {
      ...
      try {
        const response = await Promise.all(
          users.map(async (user) => {
            const res = await CLIENT_API.get(`/admin/checkUserStatus/${user._id}`);
  ...
  const handleBlockUnblock = async (userId, currentAccess) => {
    try {
      const response = await CLIENT_API.post("/admin/blockUnblockUser", {
        id: userId,
  ...
  return (
    <div className="container mx-auto p-9">
      <div className="flex justify-between items-center mb-4">
        <h4 className="text-2xl font-bold mb-4">Manage Clients</h4>
      </div>
